...
def read_excel_data(
    ...
    """엑셀 데이터를 읽어 (records, headers, sheet_title) 반환.
    records: [{열이름: 값, ..., "__source_row__": 행번호}, ...]
    """
    ...
    try:
        ...
        return records, headers, ws.title
    finally:
        wb.close()
...
def validate_rules_by_row(records: List[Dict], rules):
    """행 번호별 오류 메시지 dict 반환: {source_row: ["메시지1", ...]}"""
    row_messages: Dict[int, List[str]] = {}
    if not rules or not records:
        return row_messages

    data_cols = [c for c in records[0] if c != "__source_row__"]
    for row in records:
...
class ExcelMergeApp:
    ...
    def run_merge(self):
            ...
            for path in self.selected_files:
                ...
                try:
                    ...
                    for src_sheet_name in sheets_to_copy:
                        ...
                        # 검증: 행별 오류 메시지 수집
                        rules = self.file_rules.get(path, {})
                        records, _, _ = read_excel_data(path, src_sheet_name, hr, se)
                        row_msgs = validate_rules_by_row(records, rules)

                        # 검증 문제가 있을 때만 비고 열 추가
                        issue_count = 0
                        if row_msgs:
        ...
        self.log(f"병합 성공! 시트 {len(added_sheets)}개 추가: {', '.join(added_sheets)}", "ok")
        if total_issues:
            self.log(f"검증 문제 {total_issues}건 발생 → 각 시트 '비고' 열에 기록됨", "warn")
        if failed:
            self.log(f"일부 파일 처리 실패: {len(failed)}개", "error")
